...
const RUNTIME_VERSION = "v4-root-health-20260918";
...
<small>KRAKEN · 24/7 · PAPER ONLY</small><h1>Second Trend Paper Bot</h1><p>V4 läuft dauerhaft auf Railway. Startkapital 1.000 €, dynamische Positionsgröße 100–500 €.</p>
...
const server=http.createServer((req,res)=>{
  ...
  if(pathname==="/version")return json(res,200,{version:RUNTIME_VERSION});
  if(pathname==="/health")return json(res,200,{ok:true,version:RUNTIME_VERSION,online,running:paper.running,uptimeSeconds:Math.floor((Date.now()-startedAt)/1000)});
  if(pathname==="/state"){
    const e=equity();
    return json(res,200,{
      online,
...
server.listen(PORT,"0.0.0.0",()=>console.log("paper-v4 listening on :"+PORT,RUNTIME_VERSION));
